...
def ida_star(graph):
    ...
    num_nodes = 1
    while True:
        t, path, is_done, num_nodes = search(graph, threshold, graph.start, path, graph.start.cost, num_nodes)
        if is_done:
...
def search(graph, threshold, node, path, cost, num_nodes):
    ...
    for neighbor1 in node.neighbors:
        ...
        num_nodes += 1
        t, res_path, res_found, num_nodes = search(graph, threshold, neighbor, path + [neighbor1], cost+neighbor.cost, num_nodes)
        if res_found:
